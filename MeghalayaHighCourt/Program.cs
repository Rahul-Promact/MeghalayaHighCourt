...
class Program
{
    static async Task Main(string[] args)
    {
        ...
        chromeOptions.AddUserProfilePreference("download.default_directory", @"C:\CourtDetails\Meghalaya"); // Set your download folder path
        ...
        using (IWebDriver driver = new ChromeDriver(chromeOptions))
        {
            ...
            string url = "https://hcservices.ecourts.gov.in/ecourtindiaHC/cases/s_order.php?state_cd=21&dist_cd=1&court_code=1&stateNm=Meghalaya"; // Replace with your URL
            int maxRetries = 3;
            int retryCount = 0;
            bool success = false;
            ...
            if (success)
            {
                ...
                string[] judgeNames = new string[]
                     {
                        ...
                        "HON'BLE MR. JUSTICE S. VAIDYANATHAN, CHIEF JUSTICE",
                         "HONBLE MR. JUSTICE SANJIB BANERJEE, CHIEF JUSTICE (FORMER)",
                         "Hon'ble Chief Justice (Former)",
                         "HONBLE MR. JUSTICE RANJIT V. MORE, CHIEF JUSTICE (FORMER)",
                         "HON'BLE MR. JUSTICE BISWANATH SOMADDER, CHIEF JUSTICE (FORMER)",
                         "Hon'ble Mr Justice Ajay Kumar Mittal, Chief Justice (Former)",
                         "Hon'ble Mr Justice Mohammad Yaqoob Mir, Chief Justice (Former)",
                         "Hon'ble The Chief Justice (Former)",
                         "Hon'ble Mr Justice Uma Nath Singh CJ(Former)",
                         "HON'BLE MR JUSTICE MOHAMMAD RAFIQ, CHIEF JUSTICE (FORMER)",
                         "HON'BLE MR. JUSTICE RANJIT MORE (FORMER JUDGE)",
                         "HON'BLE MR. JUSTICE H. S. THANGKHIEW, CHIEF JUSTICE (ACTING)",
                         "HON'BLE MR. JUSTICE W. DIENGDOH",
                         "Hon'ble Mr Justice S. R. Sen (Former Judge)",
                         "Hon ble Mr Justice Ved Prakash Vaish (Former Judge)",
                         "HON'BLE MR. JUSTICE B. BHATTACHARJEE",
                         "JUSTICE T VAIPHEI (FORMER JUDGE)"
                     };
                ...
                while (judgeIndex < judgeNames.Length)
                {
                    ...
                    string fromDate = "01-01-1997";  // Adjust as needed
                    ...
                    typeOfOrdersDropdown.SelectByValue("282");
                            ...
                            try
                            {
                                ...
                                if (tableElement.Displayed)
                                {
                                    ...
                                    foreach (var row in rows)
                                    {
                                        ...
                                        casedetail.Court = "Meghalaya High Court";
                                        casedetail.Abbr = "MEG";
                                        ...
                                        if (cells.Count >= 4)
                                        {
                                            ...
                                            string downloadDirectory = @"C:\CourtDetails\Meghalaya";
                                            ...
                                            while (downloadedFile == null)
                                            {
                                                ...
                                                await Task.Delay(2000); // Wait for 2 seconds before checking again
                                            }

                                            // Wait until the file is fully downloaded and not locked
                                            while (IsFileLocked(downloadedFile))
                                            {
                                                await Task.Delay(2000); // Keep waiting if the file is still locked (downloading)
                                            }
